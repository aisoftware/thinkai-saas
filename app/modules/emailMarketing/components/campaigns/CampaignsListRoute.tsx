import { useParams, useSubmit, Form, useActionData, useNavigation, useLoaderData, useSearchParams } from "react-router";
...
export default function CampaignsListRoute() {
  ...
  const data = useLoaderData<Campaigns_List.LoaderData>();
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {i.track && (
                    <>
                      ...
                      <div>•</div>
                      <div className="text-muted-foreground text-sm">{i.recipients.filter((f) => f.clicks.length > 0).length} clicks</div>
                    </>
                  )}
                  {i.track && (
                    <>
                      <div>•</div>
                      <div className="text-muted-foreground text-sm">{i.recipients.filter((f) => f.unsubscribedAt).length} unsubscribers</div>
                    </>
                  )}
                </div>
              </div>
