...
export default function FeatureFlagForm({
  ...
  const [filters, setFilters] = useState<{ type: FeatureFlagsFilterType; value: string | null; action: string | null }[]>(
    item?.filters.map((f) => {
      return {
  ...
  useEffect(() => {
    if (!item) {
      ...
      if (name === "currency") {
        setDescription("Set default currency");
      }
    }
  }, [item, name]);

  function onSaveFilter(item: { type: FeatureFlagsFilterType; value: string | null; action: string | null }) {
  ...
  // const hasDefaultFlag = ["currency"].includes(name);
    ...
    <div>
      ...
      >
          ...
          <div className="space-y-2">
            ...
            <div>
              <div className="mb-1 flex items-center justify-between space-x-2 text-xs">
                <label className="text-muted-foreground font-medium">{t("featureFlags.filters")}</label>
                <button type="button" onClick={() => setFilters([])} className="text-muted-foreground hover:text-muted-foreground">
                  {t("shared.clear")}
                </button>
              </div>
                ...
                {filters.map((filter, idx) => {
                  return (
                    <button
                      ...
                      className="border-border hover:border-border relative block w-full rounded-lg border-2 border-dashed p-3 text-center focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
                    >
        ...
        <SessionFilterModal
          ...
          onSave={({ type, value, action }) => onSaveFilter({ type: type as FeatureFlagsFilterType, value, action: action ?? null })}
          metadata={metadata}
          onRemove={(idx) => {
