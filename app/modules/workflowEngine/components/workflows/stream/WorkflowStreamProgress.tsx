    ...
    <div>
      <div className="space-y-4">
        {execution?.blockRuns.map((blockRun, index) => {
          ...
          return (
            <div key={index} className="border-border cursor-pointer select-none overflow-hidden rounded-md border bg-white p-2 hover:bg-gray-50">
              <div className="flex justify-between space-x-2">
                <div className="flex items-center space-x-1">
                  <div className="flex items-center space-x-2 truncate">
                    <workflowBlock.icon className="text-muted-foreground h-4 w-4 flex-shrink-0" />
                    <div className="text-muted-foreground flex-shrink-0 truncate text-xs font-medium">[{workflowBlock.name}]</div>
                    {blockRun.error ? (
        ...
        {execution?.waitingBlock && (
          <Form onSubmit={onSubmitWaitingBlock}>
            <div className="space-y-1">
              {execution.waitingBlock.input.title && <label className="text-muted-foreground text-xs font-medium">{execution.waitingBlock.input.title}</label>}
