  ...
  return (
    <div>
      <Form onSubmit={onSubmit} className="inline-block w-full overflow-hidden p-1 text-left align-bottom sm:align-middle">
        ...
        <div className="mt-3">
          <h3 className="text-lg font-medium leading-6">{idx === undefined ? "New Prompt Template" : "Edit Prompt Template"}</h3>
        </div>
          ...
          <div>
            <div className="grid grid-cols-12 gap-3">
              {/* <InputMultiText
                className="w-full"
                disabled
                value={PromptFlowUtils.getUsedVariables(template.template ?? "", metadata.entities).map((f, idx) => {
                  return {
                    order: idx,
                    value: f.name,
                  };
              ...
              <div className="col-span-12 h-[calc(100vh-240px)] space-y-2 overflow-auto md:col-span-4 lg:col-span-3">
                <div>
                  ...
                  <div className="border-border flex h-full w-full flex-wrap items-center rounded-md border bg-gray-50">
                    {PromptFlowUtils.getUsedVariables(template.template ?? "", metadata.entities).length === 0 ? (
                      <div className="bg-secondary/90 border-border m-1 flex items-center rounded border px-2 py-2 text-sm">
                        <span>None</span>
                      </div>
                    ) : (
                      <>
                        {PromptFlowUtils.getUsedVariables(template.template ?? "", metadata.entities).map((item, index) => (
                          <div key={index} className="bg-secondary/90 border-border m-1 flex select-all items-center rounded border px-2 py-2 text-sm">
                            <span>{item.name}</span>
                          </div>
                        ))}
                      </>
                ...
                {/* <div>
                  <label className="mb-1 flex justify-between space-x-2 truncate text-xs font-medium text-muted-foreground">
                  ...
                  <div className="flex w-full flex-wrap items-center">
                    {PromptBuilderVariableService.getPossibleVariables(metadata.entities, templateOrder).map((item, index) => (
                      <button
                        key={index}
                        type="button"
                        className="m-1 flex w-full items-center rounded border border-border bg-secondary/90 px-2 py-2 text-sm hover:border-border hover:bg-gray-200"
                        onClick={() => {
              ...
              <div className="grid grid-cols-2 gap-4">
                <InfoBanner title="Possible Variables" text="">
                  <div className="space-y-1 ">
                    {PromptFlowUtils.getPossibleVariables(metadata.entities).map((variable) => {
                      return (
                ...
                <InfoBanner title="Used Variables" text="">
                  <div className="space-y-1">
                    {PromptFlowUtils.getUsedVariables(template.template ?? "", metadata.entities).map((variable) => {
                      return (
                        <div key={variable.name} className="flex items-center space-x-2">
