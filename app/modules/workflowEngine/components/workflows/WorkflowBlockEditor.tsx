              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1">
                  {workflowBlock.inputs.map((input) => {
                    const ifInput = input.if?.name ?? "";
                    if (ifInput) {
                      const workflowBlockInput = workflowBlock.inputs.find((x) => x.name === ifInput);
                      if (!workflowBlockInput) {
                        return null;
                      }
                      const value = block.input[workflowBlockInput.name];
                      if (!value || !input.if?.values.includes(value)) {
                        return null;
                      }
                    }
                    return (
...
function BlockInput({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setDebouncedInputValue(inputValue), timeout);
